 ...
 console.log(this);
  ...
  $('.new-tweet').on('submit', 'form', function(event) {
    ...
    let userTweet = $(this).find('textarea').val();
      ...
      if (!userTweet) {
         $(this).find('.userTweetForm').text("Empty Tweet!").addClass("form-error");
         return;

      } else if (userTweet.length > 140) {
         $(this).find('.userTweetForm').text("Too Many Characters").addClass("form-error");
         return;
      }
  ...
  $(this).find('.userTweetForm').text("Tweet sent!").addClass("form-ok");
...
//in the event of too many characters or an empty text field we will change the html elements class so we can appropriately style it to make the error clear
 ...
 // line 4 is saying upon submission of the form that submits a new tweet,
 ...
 // In this care we are serializing the json data that is being referenced iby this, which is the array of tweet objects found at /tweets
  // we gave the hide compose tweet button a class of compose so we could reference it within the anonymous function that we call .slideToggle on
  //
